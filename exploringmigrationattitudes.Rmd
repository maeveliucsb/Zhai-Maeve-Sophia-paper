...
Note: The migration flow that I have created is only for 2024. The data lives in data/dataofmigration.csv, categorized in migration.raw

The migration data is derived from the International Organization of Migration.

The chunk below is analyzing in dyad, but please feel free to do something else. 

```{r migration flows in 2024, echo=FALSE}
#Understanding migration flows only for 2024
require(data.table)
...
# Relevant Questions in World Value Survey

Country - B_Country_Alpha, numbered. Let's look at dyads
- SEA: Malaysia, MYS; Myanmar MMR
- 

Climate Concern
- Q70: Confidence: The Environmental Protection Movement
...
```{r country effect}
#Country Fixed Effect to see if there is an effect
#EnvQ70 = Crimerate124 + fixed
test.df <- data.raw

climate <- lm(Q124 ~ Q111 + B_COUNTRY_ALPHA + polity, data = test.df)
summary(climate)
...
# Handle common name issues via countrycode (add manual overrides as needed)
custom_map <- c(
  "Congo, Dem. Rep."       = "COD",
  "Congo, Rep."            = "COG",
  "Côte d’Ivoire"          = "CIV",
